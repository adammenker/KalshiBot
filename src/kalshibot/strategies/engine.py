...
class StrategyEngine:
    ...
    def evaluate_and_record(
        ...
        results = []
        for decision in self.evaluate_safely(context):
...
def record_strategy_signals_on_connection(
    ...
    *,
    ...
    paper_trade_strategy_ids = set(config.paper_trade_strategy_ids)
    for timed_check, save_result in zip(timed_checks, save_results, strict=True):
        ...
        for result in engine.evaluate_and_record(connection, context):
            if result.strategy_signal_id is not None:
                signal_ids.append(result.strategy_signal_id)
                observation_signal_count += 1
                if should_open_strategy_paper_trade(
                    result.decision,
                    paper_trade_strategy_ids=paper_trade_strategy_ids,
                ):
                    event = create_open_paper_trade(
                        connection,
                        signal_id=None,
                        strategy_signal_id=result.strategy_signal_id,
                        strategy_id=result.decision.strategy_id,
                        strategy_version=result.decision.strategy_version,
                        fair_value_provider=fair_value_provider_from_decision(
                            result.decision
                        ),
                        fair_value=result.decision.fair_value,
                        entry_policy=f"{result.decision.signal_type}_signal",
...
def should_open_strategy_paper_trade(
    decision: StrategyDecision,
    *,
    paper_trade_strategy_ids: set[str],
) -> bool:
    return (
        decision.strategy_id in paper_trade_strategy_ids
